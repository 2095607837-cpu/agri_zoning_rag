...
| 日期 | 变更 |
|------|------|
| 2026-09-01 | **CE 专用改写（五要素完整句）实验：全量替换否决，定向混合上界 +0.0032**（repair_ce_query.py / eval_repair_ce_rw.py → data/repair_ce_rw/；无生产代码改动）：用户方案——CE 专用改写句完整保留问题关系五要素（谁+做+对+条件+问）+ 专业化术语映射 + 完整句禁标题化（8 条硬约束：五要素/数值符号逐字/专名缩写/术语必采用/长度 0.6~1.4 或短题 ≤+8 字/语义一致/原因类保留依据；校验重试 1 次回退 rw[0]）。180 题生成：术语替换 67、成功 174、回退 6（Q_C23/Q_L11/Q_L01/Q_L03/Q_L12/Q_N13）、**61 题 ceq==原问**（短问句 LLM 判定无需改写，改动面收敛）。R 臂=生产召回/方案C 配额完全不变、仅 CE query 换 ce_query 句（multi 重放 quota_select+replay_top10、补算 CE 71 句 3926 对；plain 实时复刻 plain 路径；A 臂重放 sanity multi/plain 0 不一致）：**MRR 0.5932→0.5918（−0.0014）判定不达标**、R@10 90.6%→89.4%、零召回 17→19；救 3（Q_S13 0→0.111/Q_D09 0→0.333/Q_D05 0→0.125，rw 收窄损失全数救回）丢 5（Q_E24 1.0→0/Q_S07 0.1→0/Q_S15 0.5→0/Q_S23 0.1→0/Q_L07 1.0→0）升 24 降 18。丢失机制两类：① **表格碎片 CE 虚高回归 4 题**（Q_S15/Q_E24 的 R top10 被 9~10 个 len=14 碎片霸榜、gold 挤出；Q_S07/Q_S23 短 chunk 相对上升）——rw 标准化压制收益丢回，ce 句越接近原问法越触发（Q_E24 ce 与 rw 仅一字之差仍翻车，长度惩罚 0.1×log(len) 是帮凶：碎片只扣 0.026、gold 扣 0.2+）；② **口语化问法 CE 不认 1 题**（Q_L07 gold CE 0.959→0.063，rw 标准化句恰是其 rank1 的原因）。与 v2 同族但轻得多（−0.0014 vs −0.0346）。**定向混合 oracle**（仅 A 零召回 17 题换 ce 句、其余保持 A，纯重放）：MRR 0.5964（+0.0032）、R@10 92.2%、零召回 17→14、0 丢——无真实零召回判定机制，仅收益上界参考。详见"三十四" |
...
| 2026-08-25 | **Phase 3 池大小扫描**（eval_pool_size_scan.py → data/pool_size_scan_report.json；hybrid_search 拆分与 max_pool 参数化见 CHUNK_PIPELINE.md 变更日志 v2.5）：采集一次+CE 打分一次，解析式重放 50/60/80：MRR 0.5517/0.5537/0.5549、R@10 87.2/86.7/87.8%、R@10=0 23/24/22；仅 18/180 题 top10 变化，净 +1 题（救 Q_S07/Q_L02、搅黄 Q_L09），非单调，判定维持 50。本次 50 亦为 CK 移除后新基线。详见"二十五、Phase 3 池大小扫描（2026-08-25）" |
| 2026-08-25 | **CK 应用层移除后口径统一**（代码改动见 CHUNK_PIPELINE.md 变更日志 v2.5）：改写/检索/召回统一为 BASE 口径（此前生产 use_ck 默认 True 实际在注入 CK；与 2026-08-13 A/B 结论一致：CK 无正向收益），200 条改写零重跑（旧 `\|base` 缓存自动复用）；eval_v2_ck_ab.py 归档。详见"二十一、CK A/B 评测"末尾口径说明 | 
...
> 所有配置均使用 `BAAI/bge-small-zh-v1.5`（512-dim）、Append 架构、RRF dense/BM25=0.7/0.3、alpha=0.2（2026-08-26 起为 0.3），除非特别注明。
> **当前生产配置**：2026-07-29 三池分离 Rewrite（chunks_split, Evidence Merge, Hit Boost 0.002）+ 2026-08-26 CE 融合 α 0.2→0.3（minmax 不变，max_pool=50）。
...
| **15** | **2026-08-26 α=0.3 新基线** | chunks_split | Evidence Merge | 180 | **0.5763** | 78.9% | **89.4%** | 42.8% | **19** | — | 解析重放 |
...
## 二十七、CE 融合 α 扫描与归一化验证（2026-08-26）

**背景**：26.5 指出 C 类 17 题的主瓶颈是 CE/Prior 融合，并观察到两个具体症状：Q_C25 的 CE min-max 归一化被池内 outlier 压缩（ce_rank=8 → final=23）、Q_L07/Q_L08 prior=1 被 0.8 权重的 CE 拉到 final 13/14。本节按"先定权重、再定归一化"的顺序做两个单因素实验。

**方法**：180 题生产口径采集一次（_collect_candidates + Phase 3 max_pool=50 + CE 逐对打分一次），全部组合解析式重放。基线一致性校验 180/180（minmax@α=0.2 重放逐题等于生产 top10）。原始分（池组成 + priors + ce_adj）全量落盘 `data/ce_norm_cache.json`，此后任何组合零模型成本。无 LLM 调用（改写缓存命中）。

### 27.1 实验 1：α 扫描（归一化固定 minmax）

final = α × prior + (1-α) × CE_norm，α = prior 权重：

| α | MRR | R@5 | R@10 | Top1 | R@10=0 | 救回 | 搅黄 | 净 |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 0.0 | 0.5103 | 70.6% | 83.9% | 37.8% | 29 | 2 | 8 | -6 |
| 0.1 | 0.5388 | 70.6% | 85.6% | 40.0% | 26 | 1 | 4 | -3 |
| 0.2（旧默认） | 0.5517 | 73.9% | 87.2% | 40.6% | 23 | — | — | — |
| **0.3** | **0.5763** | **78.9%** | **89.4%** | **42.8%** | **19** | **4** | **0** | **+4** |
| 0.4 | 0.5919 | 79.4% | 88.9% | 43.9% | 20 | 5 | 2 | +3 |
| 0.5 | 0.6093 | 81.1% | 88.9% | 47.2% | 20 | 5 | 2 | +3 |
| 0.6 | 0.6033 | 81.1% | 87.8% | 46.1% | 22 | 5 | 4 | +1 |
| 0.7 | 0.6004 | 80.6% | 86.7% | 46.1% | 24 | 6 | 7 | -1 |

**判定：α=0.3 是唯一零搅黄的正收益点**，设为新生产默认：

- R@10 87.2→89.4%（+2.2pp）、MRR 0.5517→0.5763（+0.0246）、R@10=0 23→19、Top1 +2.2pp
- 救回 4 题全部为 C 类：Q_C08、Q_D26、Q_SR04（near-miss，margin 0.0015~0.0138）、Q_L08（prior-strong prior=1）——正是 26 节预判的"prior 权重打不过 CE"人群
- 无搅黄：0.2→0.3 的边界内 CE 语义能力未被压制
- α≥0.4 出现搅黄（Q_D19、Q_S26），α≥0.6 净收益转负（prior 不是 ground truth，过度信任损伤 CE）
- CE-hard 5 题任何 α 均不可救（调权重对其无效），与 26 节结论一致
- 注意：本次为样本内（180 题）结果；α=0.3 的零搅黄特性使其优于 MRR 更高但引入搅黄的 0.4/0.5

### 27.2 实验 2：归一化扫描（固定 α=0.3）

| 方法 | MRR | R@5 | R@10 | Top1 | R@10=0 | 救回 | 搅黄 | 净 |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| **minmax（生产）** | 0.5763 | 78.9% | **89.4%** | 42.8% | **19** | 4 | **0** | **+4** |
| rank | 0.5760 | 75.6% | 88.9% | 43.9% | 20 | 4 | 1 | +3 |
| zsigmoid | 0.5946 | 79.4% | 88.3% | 45.0% | 21 | 4 | 2 | +2 |
| softmax T=0.05 | 0.5576 | 76.1% | 85.6% | 40.6% | 26 | 4 | 7 | -3 |
| softmax T=0.1 | 0.5780 | 78.3% | 85.6% | 42.8% | 26 | 5 | 8 | -3 |
| softmax T=0.2 | 0.5972 | 78.3% | 84.4% | 45.6% | 28 | 6 | 11 | -5 |

**判定：归一化维持 minmax，不换**：

- Q_C25 的 minmax outlier 压缩现象是真实的（rank 确实单独救回 Q_C25），但属局部问题：换归一化在别处搅黄的更多（rank 搅 1、zsigmoid 搅 2）
- softmax 三档均显著变差：概率过度集中到 top-1，等价于放大 CE 权重、摧毁 prior 制衡（α 等效漂移，回到 27.1 中 α<0.2 的坏区）
- zsigmoid 的 MRR/Top1 略高但 R@10 和净变化都不如 minmax

### 27.3 代码改动

| 文件 | 修改 |
|------|------|
| `hybrid_search.py` | `search_multi_query` / `search` 的 `alpha` 默认值 0.2 → 0.3；无 reranker 回退融合 0.8/0.2 → 0.7/0.3 |
| `eval_pool_size_scan.py` / `eval_pool50_failure_classify.py` | ALPHA 常量同步 0.3（重放口径跟随生产） |
...
## 三十二、v2 合体句（文档风格）与 M 臂隔离实验：修复 query 方向正式否决（2026-08-30）

### 32.1 背景与设计

31 节留下的问题：修复句 CE 只拿到"不收窄"的一半，拿不到"标准化压制"的另一半。v2 假设：v1 最小修复禁用重排/压缩，多数题只改 1-2 词，CE 端拿不到 rw 的标准化压制；v2 允许句式重构（问句→标题风格）+ 词汇文档化 + 适当增量（疑问词隐含维度显式化，如"为什么"→"区划依据"），同时术语硬对齐 + 语义要素保真，目标"1+1>2"。**明确不要求紧凑度**（紧凑度→压缩→丢语义→rw 收窄失败模式复现）。

V2_PROMPT（repair_v2_sample.py，全量生成与抽查共享）：9 条硬约束（数值/符号逐字保真、专名与缩写保真、术语不回退、禁新概念、语义要素不丢、允许隐含要素显式化、原因类必须保留依据要素、列举类保留列举语义、方法/判断类保留方法/判断语义）+ 5 组改写示例。生成流程（repair_query_v2.py）：term_replace（程序化）→ LLM（temperature=0，json_mode）→ validate_v2（数字/符号集合一致、长度比 0.4~2.0、术语回退）→ 违规重试 1 次 → 仍违规回退 mapped。180 题全量：改写 177、术语替换 67、重试 5、残留违规 1。

**temperature=0 不稳定**（8 题抽查期间发现）：同一问题多次生成得到不同句子（Q_S15 前缀"新疆冬小麦气候区划中"时有时无、Q_N20"统计"出现又消失），CE 结果翻转（Q_S15 0.5→0）。根因：DeepSeek chat 接口无 seed 参数，temperature=0 服务端 greedy 解码仍受批处理调度/浮点归约数值噪声影响；自由重写的合法解空间大，模型在多个合法句之间被噪声选择；违规重试协议放大噪声（首轮违规集合不同→重试 prompt 不同→末句更发散）。已实施的缓解：一次性生成 + 缓存锁定（评测可复现，但锁死的是单次抽签结果——Q_S15 的 0.5→0 即抽中坏句锁死）。修法评估（未实施）：① 协议层锚定 mapped 最小编辑缩小解空间；② best-of-N（3 候选→validate_v2 过滤→与 rw 的 n-gram 重合度最高者，确定性规则选优，540 次调用一次性成本）；③ 本地模型固定种子（重，不采用）。评估结论：0.5586 含抽签噪声，但主体损伤（分差扁平/压制丢失/长度惩罚）与抽签无关，修完确定性 v2 仍低于 A，不会翻案。

### 32.2 Stage 2 v2 全量 C 臂（v2 召回 + v2 CE）

口径与 31 节 C 臂一致（主查询=修复句、rw 移除、force_multi、配额 30），仅 CE query 从 v1 句换 v2 句。

| 指标 | A 生产 | C_v1（31 节） | C_v2 |
|---|---:|---:|---:|
| MRR | 0.5932 | 0.5716 | 0.5567（−0.0365） |
| R@5 / R@10 | 77.2% / 90.6% | 75.0% / 89.4% | 74.4% / 88.9% |
| 零召回 | 17 | 19 | 20 |

救 4（Q_S13/Q_D05/Q_D09/Q_SR07）/ 丢 7（Q_E24 1.0→0、Q_L01 1.0→0、Q_S15 0.5→0、Q_T16/Q_N20/Q_T24/Q_T28）/ 升 26 / 降 36。v2 比 v1 更差（−0.015）：文档风格化 + 增量没有兑现"1+1>2"，且 177/180 大面积改写（v1 仅 9 题 LLM 修复）带来大范围回归。

### 32.3 M 臂（生产召回 + CE query=v2）

设计（eval_repair_m.py）：候选/配额/池与 A 完全一致（生产 candidates + 方案C + 配额 20/10 + 池上限 60/50），仅 CE query = v2 句。CE 原始分复用 data/repair_stage2_v2/ce_scores.json（9672 对），生产池差集补算（新补 987 对，约 7 分钟）。plain 题（58 道 kw-only）无候选缓存，复刻 search() plain 路径实时检索（RRF top30 ∪ Dense top5）+ CE query=v2。A 臂 ids 直接读 2×2 存量 top10（planC|rw）。

| 指标 | A 生产 | M |
|---|---:|---:|
| MRR | 0.5932 | 0.5586（−0.0346） |
| R@5 / R@10 | 77.2% / 90.6% | 74.4% / 90.0% |
| secMRR | 0.6078 | 0.5791 |
| 零召回 | 17 | 18 |

救 2（Q_D05 0→0.143、Q_D09 0→0.167）/ 丢 3（Q_E24 1.0→0、Q_S15 0.5→0、Q_N20 0.125→0）/ 升 19 / 降 35。L 类重伤：Q_L01 1.0→0.167、Q_L07 1.0→0.2、Q_L08 1.0→0.2、Q_L06 1.0→0.5——长 gold chunk 被 0.1×log(len) 惩罚（0.52~0.66）反噬，v2 造不出原始分差时碎片（len≈14 几乎不罚）霸占 top10。

专项（rw 收窄丢 3 / 压制救 5）：丢 3 里仅 Q_D09 救回（0→0.167），Q_S13/Q_D12 仍零；救 5 里 Q_S15 丢回（0.5→0）、Q_E24 丢回（1.0→0）、Q_SR03 降（0.333→0.25），Q_S07/Q_S23/Q_SR06 保持。

### 32.4 三臂分解：退化 95% 在 CE 端

| 臂 | 召回主查询 | CE query | MRR |
|---|---|---|---|
| A | rw[0] | rw[0] | 0.5932 |
| M | rw[0] | v2 | 0.5586 |
| C_v2 | v2 | v2 | 0.5567 |

- CE 端损失 = A − M = **−0.0346**（占 Stage 2 总退化 0.0365 的 95%）
- 召回端损失 ≈ M − C_v2 = +0.0019（接近零；混有 C 配额 30 vs M 配额 20 差异，真实值略大于此但量级不变）

推翻 31 节"Stage 2 退化里召回损失占大头"的推测：**主查询换 v2 在召回端几乎无损，SubQ/kw 多通道承担了召回**；主查询的角色在 CE 阶段才凸显。v2 的全部问题在 CE 端。

### 32.5 机制定论：v2 在两端为什么不行

**召回端**（几乎无损失，但也无收益）：embedding 是语义平均不是并集，合体句 = 原问与 rw 的插值，把最优的 rw 主查询稀释回原问方向；但 SubQ/kw 通道已覆盖召回，主查询影响被稀释——"召回端为什么不行"的正确答案是"召回端其实没怎么损失，只是也没有换它的理由"。

**CE 端**（−0.0346 主因）：
1. 标准化压制丢失：rw 句短而聚焦、与文档句同长度分布，碎片（len≈14 表格行）相对 rw 的 CE 原始分被压低；v2 句更长、要素更多，碎片沾上一个术语就得分，虚高压不住（丢 3 里 Q_S15/Q_E24 即此）。
2. 分差扁平化：CE 排序靠分差不靠平均相似度，合体句信息丰富 → 与每个 chunk 都"沾边"，原始分分布变平，排名被 prior 和长度惩罚接管。
3. 长度惩罚反噬：final=0.3×prior+0.7×minmax(CE_raw−0.1×log(len)) 对长 gold chunk 结构性不利，v2 造不出原始分差时碎片上位（L 类重伤 4 题）。
4. 收益子集小、成本摊全量：177/180 题 CE query 全变，但配额/prior/alpha 均围绕 rw 调优；收益集中在 rw 失败的少数题（专项 8 题救 6、平均 rr 0.136），成本摊到 180 题 → 净负。抽查与全量的落差即此结构。

### 32.6 结论

1. **v2 合体句方向正式否决**：文档风格化 + 隐式增量未能实现"1+1>2"，CE query 换 v2 全量 −0.0346。
2. **生产 v2.9（rw-CE + 方案C）确认为当前全局最优**：召回端生产原样（rw 通道不可替代），CE 端 rw[0]（标准化压制是 CE 端核心价值，收窄副作用是同一机制两面的较小代价）。
3. 修复 query 实验链关闭：v1 最小修复（0.5716）→ v2 文档风格（0.5567）→ M 隔离（0.5586），全部负于 rw-CE 基线。
4. 后续候选方向（未决）：① CE 长度惩罚修复（LAMBDA_LEN 扫描或改为相对文档平均长度）——结构性损伤，不动 query 端；② 定向混合（rw-CE 零召回时 fallback v2-CE）——Q_D05/Q_D09 证明 v2 在个别题真实有效，只影响 17 道零召回题，只赚不赔但救回面窄。

### 32.7 代码与数据

| 文件 | 说明 |
|------|------|
| `repair_v2_sample.py` | V2_PROMPT（9 条硬约束 + 5 组示例）+ 6 题抽查 |
| `repair_query_v2.py` | v2 全量生成器（term_replace → LLM → validate_v2 → 重试 → 回退 mapped，缓存 data/repair_cache_v2.json） |
| `eval_repair_stage2.py` | --cache/--out-dir/--label 参数化（v2 重跑复用；repair 键 v1/v2 探测） |
| `eval_repair_m.py` | M 臂回放（生产候选+配额 20/10，CE query=v2；plain 题实时检索复刻；A 臂读 2×2 存量 top10） |
| `data/repair_stage2_v2/report.json` | Stage 2 v2 C 臂指标 + 逐题差异 |
...
## 三十四、CE 专用改写（五要素完整句）实验：全量替换否决、定向混合上界 +0.0032（2026-09-01）

### 34.1 背景与口径

rw-CE（三十节）机制两面性：rw 标准化句压制表格碎片 CE 虚高分（救 5），但主题收窄副作用（丢 3：Q_S13/Q_D09/Q_D12）。用户提出 CE 专用改写方向：CE query 用完整保留问题关系五要素（谁+做什么+对谁+什么条件+问什么）的专用句——既恢复语义完整性（救回收窄损失），又通过专业化表达+完整句规避 v2 合体句的三条失败路径（标题化分差扁平化、标准化压制丢失、长度惩罚反噬）。

生成器 repair_ce_query.py：程序化术语替换（复用 repair_query.term_replace，67 题）→ LLM 改写（CE_RW_PROMPT，temperature=0，8 条硬约束）→ 校验（数字/符号集合一致、长度 0.6~1.4 或短题 ≤原问+8 字、术语不回退、禁标题化、五要素非空）→ 违规重试 1 次 → 仍违规回退 rw[0]。180 题：成功 174、回退 6（Q_C23/Q_L11/Q_L01/Q_L03/Q_L12/Q_N13）、**61 题 ceq==原问**（短问句 LLM 判定原始问法已是最优，意外收敛了改动面——这是损失远小于 v2 的主因）。

R 臂回放 eval_repair_ce_rw.py：生产召回与方案C 配额完全不变，仅 CE query 换 ce_query 句——multi 题用 quota_select（生产候选）+ replay_top10 重放，CE 原始分逐题补算（71 个新句 3926 对；51 题句子命中生产缓存 0 新增；plain 题 58 道实时复刻 plain 路径 RRF top30 ∪ Dense top5 不落盘）。A 臂=生产存量 planC|rw；sanity：A 臂重放 vs 存量 multi/plain 逐位一致 0 题。

### 34.2 全量结果与判定

| 臂 | MRR | R@5 | R@10 | 零召回 | hit |
|---|---|---|---|---|---|
| A 生产（rw-CE） | **0.5932** | 77.2% | **90.6%** | **17** | 241 |
| R（ce_query 五要素句） | 0.5918 | 77.2% | 89.4% | 19 | 250 |

救 3 丢 5 升 24 降 18 → **MRR 0.5918 < 0.5932，判定不达标**；CE 端维持 rw[0]，方向否决。

### 34.3 逐题机制

- **救 3 = rw 收窄损失全数救回**：Q_S13 0→0.111、Q_D09 0→0.333、Q_D05 0→0.125。逐题机制：Q_D09 gold CE 池内排名 19→4（0.245→0.571，rw 句压掉了"各省做了哪些调整"的问题结构，ce 恢复后 CE 重新认出 3 个 gold 进 top10）；Q_D05 gold CE 惩罚后分 0.490→0.701（raw 0.724→0.935）；Q_S13 是相对效应（gold CE 略降 0.996→0.983，但竞品长 chunk s3_p0/s1_p0 掉幅更大，gold s2_p0 挤进第 9）。
- **丢 5 分两类**：
  - **表格碎片 CE 虚高回归 4 题**：Q_S15 0.5→0（R top10 被 9 个 len=14 碎片霸榜，gold len=756 整个挤出，A 里仅 1 个碎片且 gold 排第 2）、Q_E24 1.0→0（R top10 全部 10 个 len=14 碎片——注意 ce 句与 rw 仅"区划报告/气候区划报告"一字之差仍翻车，碎片虚高是 CE 的病理敏感区；长度惩罚 0.1×log(len) 是帮凶：碎片只扣 0.026、gold len=180 扣 0.226）、Q_S07 0.1→0（gold CE 0.948→0.851 从第 10 掉出）、Q_S23 0.1→0（gold s37 第 10 掉出，短 chunk 相对上升）；
  - **口语化问法 CE 不认 1 题**：Q_L07 1.0→0——ce 句保留"为什么…有的地方…"口语框架，gold（陕西苹果区划报告段落）CE 从 0.959 崩到 0.063（池内第 15），rw 标准化句"苹果种植的气候适宜性差异及其区划依据是什么？"恰是其 A 臂 rank1 的原因（与已知 failure mode"口语问法与文档语言语义鸿沟"同族，rw 标准化正是为这类题服务）。
- **与 v2 的对比**：机制同族（碎片虚高回归 + 标准化压制丢失）但损失幅度 −0.0014 vs −0.0346（95% 差距）——完整句约束 + 61 题原句不动共同收敛了破坏面。结论不变：**恢复收窄损失与保留压制收益不可兼得**，全量替换任何方向都划不来。

### 34.4 定向混合 oracle（收益上界）

仅对 A 零召回 17 题启用 ce 句、其余保持 A（纯重放 per_q.json，无真实判定机制）：

| 臂 | MRR | R@5 | R@10 | 零召回 |
|---|---|---|---|---|
| A | 0.5932 | 77.2% | 90.6% | 17 |
| H（oracle 混合） | **0.5964** | **77.8%** | **92.2%** | **14** |

+0.0032 全部来自 3 题救回（0.111+0.333+0.125=0.569，/180=+0.0032），0 丢 0 降。**生产无零召回先知**（无法预知哪题该换 ce 句），该数字只作 CE 改写方向的收益上界参考，不构成上线方案；若未来有零召回预测信号（如检索 prior 置信度），此方向可复活为混合策略。

### 34.5 代码与数据

| 文件 | 说明 |
|------|------|
| `repair_ce_query.py` | CE 专用改写生成器（五要素 prompt + 8 硬约束校验 + 重试回退） |
| `eval_repair_ce_rw.py` | R 臂回放脚本（multi 重放 + plain 实时复刻 + A 臂 sanity + 逐题 dump） |
| `data/repair_ce_rw/cache.json` | 180 题 ce_query 句 + 要素 + 校验记录 |
| `data/repair_ce_rw/ce_scores.json` | 新句 CE 原始分（71 句 3926 对） |
| `data/repair_ce_rw/report.json` | A/R 指标 + 救丢升降 + sanity |
| `data/repair_ce_rw/per_q.json` | 逐题 A/R top10 ids（混合重放用） |
