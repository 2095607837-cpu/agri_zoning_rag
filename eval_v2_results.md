...
- **救 3 = rw 收窄损失全数救回**：Q_S13 0→0.111、Q_D09 0→0.333、Q_D05 0→0.125（ce 句恢复完整语义范围，主题收窄副作用消失）。
- **丢 5 分两类**：
  - rw 标准化压制收益丢回 3 题：Q_S07 0.1→0、Q_S15 0.5→0、Q_S23 0.1→0（压制救 5 中的 3 题归零；同组 Q_SR03 0.333→0.25、Q_SR06 0.125→0.1 降未丢）——ce 句显著长于 rw（Q_S23 54 vs 27 字、Q_S15 31 vs 20 字），len≈14 表格碎片 CE 虚高分重新抬头；
  - CE 长度惩罚反噬 2 题：Q_E24 1.0→0、Q_L07 1.0→0（L 类重伤，与三十二节 v2 机制同族）。
- **与 v2 的对比**：机制同族（标准化压制丢失 + 长度惩罚反噬）但损失幅度 −0.0014 vs −0.0346（95% 差距）——完整句约束 + 61 题原句不动共同收敛了破坏面。结论不变：**恢复收窄损失与保留压制收益不可兼得**，全量替换任何方向都划不来。
